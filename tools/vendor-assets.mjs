...
/* ---------------------------------------------------------------- glyph module
   ...
   The SVGs are also emitted as a JS module of path data.

   Fetching an icon at runtime needs an asset base URL, which the design-system
   bundle derives from its own <script src> — a trick that does not exist in a
   bundled React app, and which costs a request and a frame of placeholder per
   glyph either way. Every Material Symbol here is a single <path> on one shared
   viewBox, and so is every Simple Icon, so only the path data has to be stored:
   178 glyphs in ~60KB raw, and they render synchronously with no network and no
   configuration.

   The files in icons/ and brand/ stay — they are the source this is generated
   from, and Icon still falls back to fetching for any name not in here. */
...
for (const name of copied) {
  const d = pathOf(readFileSync(join(ASSETS, 'icons', name + '.svg'), 'utf8'));
  if (d) glyphEntries.push('  ' + JSON.stringify(name) + ': ' + JSON.stringify(d) + ',');
}
const brandEntries = [];
for (const slug of brandCopied) {
  const d = pathOf(readFileSync(join(ASSETS, 'brand', slug + '.svg'), 'utf8'));
  if (d) brandEntries.push('  ' + JSON.stringify(slug) + ': ' + JSON.stringify(d) + ',');
}

writeFileSync(
  join(PROJECT, 'components', 'core', 'glyphs.js'),
  [
    ...
    'export function registerGlyphs(extra, brand) {',
...
console.log('glyph  ' + glyphEntries.length + ' + ' + brandEntries.length + ' path data written to components/core/glyphs.js');
