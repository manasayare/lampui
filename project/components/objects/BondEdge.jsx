...
export function BondEdge({ from, to, state = 'valid', energy = false, environment = 'draft', junction = true, className = '', ...rest }) {
  ...
  return (
    <g className={['lamp-bond', energy && 'lamp-bond--energy', className].filter(Boolean).join(' ')} {...rest}>
...
export function BondLayer({ width, height, children, ...rest }) {
  return (
    <svg width={width} height={height} style={{ position: 'absolute', left: 0, top: 0, overflow: 'visible', pointerEvents: 'none', zIndex: 2 }} aria-hidden="true" {...rest}>
      {children}
    </svg>
  );
}
