...
export function ChatAvatar({ role = 'agent', initials, glyph, state = 'idle', size = 26 }) {
  if (role === 'user') {
  ...
  return (
    <span className="lamp-cm__avatar" style={{ width: size, height: size }}>
...
export function ChatActivity({ items = [], max = 4, onSelect }) {
...
export function ChatCitation({ index, source, scope, onClick }) {
  return (
    <button type="button" className="lamp-cite" onClick={onClick} title={[source, scope].filter(Boolean).join(' · ')} aria-label={'Evidence ' + index + (source ? ': ' + source : '')}>
...
export function ChatDayDivider({ label }) {
  return <div className="lamp-chat__day"><span>{label}</span></div>;
}
...
export function ChatMessage({
  ...
  return (
    <article className={['lamp-cm', 'lamp-cm--' + role, grouped && 'lamp-cm--grouped', state === 'acting' && 'lamp-cm--acting', failed && 'lamp-cm--failed', className].filter(Boolean).join(' ')} {...rest}>
