...
export function VariableToken({ name, kind = 'variable', children, ...rest }) {
  const label = children || name;
  return <span className={'lamp-vartoken' + (kind === 'entity' ? ' lamp-vartoken--entity' : kind === 'missing' ? ' lamp-vartoken--missing' : '')} {...rest}>
    <Icon name={kind === 'entity' ? 'alternate_email' : kind === 'missing' ? 'warning' : 'data_object'} size={10} />{label}
  </span>;
}

export function PromptEditor({ label = 'Instructions', children, tokens = [], footer, onInsert, ...rest }) {
  return (
    <div className="lamp-prompt" {...rest}>
