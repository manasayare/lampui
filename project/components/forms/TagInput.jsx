...
export function TagInput({ values = [], onChange, placeholder = 'Add…', mono = false, ...rest }) {
  ...
  return (
    <div className={['lamp-taginput', focus && 'lamp-taginput--focus'].filter(Boolean).join(' ')} {...rest}>
      ...
      <input value={draft} placeholder={placeholder} onFocus={() => setFocus(true)} onBlur={() => { setFocus(false); commit(); }}
