...
export const InspectorSection = /* @__PURE__ */ Object.assign(/* @__PURE__ */ React.forwardRef(function InspectorSection({ label, count, actions, collapsible = true, defaultOpen = true, advanced = false, children, className = '', ...rest }, ref) {
  const [open, setOpen] = React.useState(advanced ? false : defaultOpen);
  const isOpen = collapsible ? open : true;
  return (
    <section ref={ref} className={['lamp-insp-sec', !collapsible && 'lamp-insp-sec--static', advanced && 'lamp-insp-sec--advanced', className].filter(Boolean).join(' ')} {...rest}>
...
export const PropertyRow = /* @__PURE__ */ Object.assign(/* @__PURE__ */ React.forwardRef(function PropertyRow({ label, value, mono = false, stack = false, inherited = false, empty, children, className = '', ...rest }, ref) {
  const content = children != null ? children : (value == null || value === '' ? <span className="lamp-prop__empty">{empty || 'Not set'}</span> : value);
  return (
    <div ref={ref} className={['lamp-prop', stack && 'lamp-prop--stack', inherited && 'lamp-prop--inherited', className].filter(Boolean).join(' ')} {...rest}>
