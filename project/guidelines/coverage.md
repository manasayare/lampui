...
| TokenUsage, ContextBreakdown, CacheRate, CostByAgent, CostByPlaybook, LatencyBreakdown | `MemoryUsageBar` + `Viz type="hbar"` + `MetricCard` |
...
| Trace, TraceSpan, TraceEvent, RunStep, SimulationStep, SimulationTimeline | `RunTimeline` / `RunStep` |
...
| MemoryCard, MemoryValue, MemorySource, MemoryEvidence, MemoryFreshness, MemoryConflict, MemoryCorrection, lifecycle states | `MemoryFact` (`type`, `scope`, `state`, `contradictions`) + `MemoryBadge` + `MemoryScope` |
| ObservedStep, ProcessVariant, ProcessException, ProcessHandoff, ProcessRework, ProcessApproval | `ObservedProcess` step `kind` |
| LearnedPattern, ObservedPattern, SuggestedPlaybook, ProcessProposal, UncertainFact, ConfirmationRequest | `LearnedPattern` `kind` |
