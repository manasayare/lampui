...
| `components/core/glyphs.js` | Generated — icon path data, inlined so icons render synchronously with no request. |
| `../packages/react/` | `@lamp/design-system`, the npm package built from these same sources. See §19. |
...
## 19. Consuming this from a React app

`components/` is the source; **the design-system bundle and the npm package are two builds of it**, so a fix lands in both and neither is a fork.
...
One stylesheet carries the tokens, both themes and every component, with the Geist faces beside it. Theme and density stay attributes on the root element — `data-theme="dark"`, `data-density="compact"` — so switching either costs no React render.

**Icons are inlined.** `<Icon name="hexagon" />` renders synchronously from path data bundled with the package: no request, no placeholder frame, no CDN, no asset base to configure, and correct under SSR. The cost is about 15kB gzipped for the whole set, which is why even a Button-only app carries it. Any name outside the set still works by falling back to a fetch for that one glyph.

**GSAP is injected, not imported.** The canvas runs on GSAP Draggable, but the package never imports it — that would force the dependency on an app that only wanted a Button, and no bundler could shake it out. The consumer hands it over:
